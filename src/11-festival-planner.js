...
export function createFestivalManager() {
  ...
  return {
    addFestival(name, date, type) {
      if (!type || !["religious", "national", "cultural"].includes(type)) return -1
      if (!date || typeof date !== "string") return -1
      if (!name || typeof name !== "string") return -1
      
      const exists = festivals.some((obj) => obj.name === name)
      if (exists) return -1
      festivals.push(
        {
          name,
    ...
    getByType(type) {
      return festivals.filter((obj) => obj.type === type).map((obj) => ({...obj}))
    },

    getUpcoming(currentDate, n=3) {
      return festivals.filter((obj) => {
        return obj.date > currentDate
      }).sort((a,b) => a.date.localeCompare(b.date)).slice(0,n).map((obj) => ({...obj}))
    },
    getCount() {
      return festivals.length
    }
